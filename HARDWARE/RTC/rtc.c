...
#include "task.h"
#include "queue.h"
TaskHandle_t StartRtcHandler;		//任务句柄
...
u8 RTC_Init(void)
{
	...
	if (BKP_ReadBackupRegister(BKP_DR1) != 0x5050)		//从指定的后备寄存器中读出数据:读出了与写入的指定数据不相乎
		...
		RTC_WaitForLastTask();	//等待最近一次对RTC寄存器的写操作完成
		RTC_Set(2022,11,9,9,20,00);  //设置时间	
...
//	u8 ret=0;
	...
	{		
		...
		if(work_time.work_time_flag==0)	//如果现在不是工作时间，则进去判断
		{	
		...
		else if(work_time.work_time_flag ==1)	//如果是工作时间
		{
			if(calendar.week<Cjson_Buf.Cjson_Buffer_Data[work_time.which_working_time].week_start && calendar.week>Cjson_Buf.Cjson_Buffer_Data[work_time.which_working_time].week_end)
			{
...
{
		//创建RTC任务
	xTaskCreate((	TaskFunction_t) RTC_task,					//任务1函数
						(const char * 	)"RTC_task",			//任务1函数名
						(uint16_t 			)RTC_STK_SIZE,		//任务1堆栈数
						(void * 				)NULL ,			//传入函数参数
						(UBaseType_t 		)RTC_TASK_PRIO,		//任务1优先级
						(TaskHandle_t * )&StartRtcHandler );	//取址
